...
interface ServiceForm { name: string; price: string; duration: string }
...
export default function ServicesScreen() {
  ...
  const { mutate: saveService, isPending: saving } = useMutation({
    mutationFn: async () => {
      if (!form.name || !form.price || !form.duration) throw new Error('Please fill in all fields');
      ...
      if (editService) {
        ...
        if (!business?.id) throw new Error('Business profile not found. Please create your business first.');
    ...
    onError: (err: unknown) => Alert.alert('Error', err instanceof Error ? err.message : 'Failed to save'),
  });
  ...
  const openEdit = (svc: Service) => {
    setEditService(svc);
    setForm({ name: svc.name, price: String(svc.price), duration: String(svc.duration_minutes) });
    setShowModal(true);
  };
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {!business ? (
        <View style={styles.center}>
          ...
          <Text style={[styles.emptyTitle, { color: colors.text, fontFamily: 'Inter_600SemiBold' }]}>Business setup pending</Text>
          <Text style={[styles.emptyText, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>An admin must register your business before services can be managed.</Text>
        </View>
      ) : isLoading ? (
        ...
        <FlatList
            ...
            <ServiceCard
              ...
              onDelete={() =>
                Alert.alert('Delete Service', `Delete "${item.name}"?`, [
                  ...
                  { text: 'Delete', style: 'destructive', onPress: () => deleteService(item.id) },
                ])
              }
            />
          )}
          ListEmptyComponent={
            <View style={styles.center}>
              ...
              <Text style={[styles.emptyTitle, { color: colors.text, fontFamily: 'Inter_600SemiBold' }]}>{t('noServices')}</Text>
              <Text style={[styles.emptyText, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>{t('noServicesDesc')}</Text>
              <Pressable style={[styles.ctaBtn, { backgroundColor: colors.primary }]} onPress={openAdd}>
                <Text style={[styles.ctaText, { fontFamily: 'Inter_600SemiBold' }]}>{t('addService')}</Text>
              </Pressable>
            </View>
          }
        />
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <Pressable style={styles.overlay} onPress={() => setShowModal(false)}>
            <Pressable style={[styles.sheet, { backgroundColor: colors.card }]} onPress={() => {}}>
              ...
              <FormField label={t('serviceName')} value={form.name} onChangeText={(v) => setForm((f) => ({ ...f, name: v }))} placeholder="e.g. Haircut" colors={colors} />
              <FormField label={`${t('servicePrice')} ($)`} value={form.price} onChangeText={(v) => setForm((f) => ({ ...f, price: v }))} placeholder="25" keyboardType="decimal-pad" colors={colors} />
              <FormField label={`${t('serviceDuration')} (min)`} value={form.duration} onChangeText={(v) => setForm((f) => ({ ...f, duration: v }))} placeholder="30" keyboardType="number-pad" colors={colors} />

              <View style={styles.sheetBtns}>
                <Pressable style={[styles.cancelBtn, { borderColor: colors.border }]} onPress={() => setShowModal(false)}>
                  <Text style={[styles.cancelText, { color: colors.text, fontFamily: 'Inter_500Medium' }]}>{t('cancel')}</Text>
                </Pressable>
                <Pressable
                  ...
                  disabled={saving}
                >
                  {saving ? <ActivityIndicator size="small" color="#fff" /> : <Text style={[styles.saveText, { fontFamily: 'Inter_700Bold' }]}>{t('save')}</Text>}
                </Pressable>
              </View>
            </Pressable>
          </Pressable>
...
function FormField({ label, value, onChangeText, placeholder, keyboardType, colors }: {
  ...
  return (
    <View style={styles.field}>
      <Text style={[styles.fieldLabel, { color: colors.text, fontFamily: 'Inter_500Medium' }]}>{label}</Text>
...
const styles = StyleSheet.create({
  ...
  ctaBtn: { paddingHorizontal: 24, paddingVertical: 12, borderRadius: 10, marginTop: 8 },
  ...
  fieldInput: { borderRadius: 10, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 12 },
