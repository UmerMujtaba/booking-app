...
async function fetchLatestUpdateRequest(ownerId: string): Promise<BusinessUpdateRequest | null> {
...
interface BizForm { name: string; category: string; bio: string }
...
export default function OwnerProfileScreen() {
  ...
  const [form, setForm] = useState<BizForm>({ name: '', category: 'Barber', bio: '' });

  useEffect(() => {
    if (business) {
      setForm({ name: business.name, category: business.category, bio: business.bio ?? '' });
    }
  }, [business]);

  const { mutate: requestBusinessUpdate, isPending: saving } = useMutation({
    mutationFn: async () => {
      if (!business) throw new Error('Your business must be registered by an admin first.');
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['owner-business-update-request'] });
      Alert.alert('Submitted', 'Your changes were submitted for admin approval.');
    },
    onError: (err: unknown) => {
      console.log("🚀 ~ OwnerProfileScreen ~ err:", err)
      return Alert.alert('Error', err instanceof Error ? err.message : 'Request failed');
    },
  });
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <View style={styles.ownerCard}>
          ...
          <View>
            <Text style={[styles.ownerName, { color: colors.text, fontFamily: 'Inter_600SemiBold' }]}>
              {profile?.full_name}
            </Text>
            <View style={[styles.roleBadge, { backgroundColor: colors.accent + '18' }]}>
              <Text style={[styles.roleText, { color: colors.accent, fontFamily: 'Inter_500Medium' }]}>{t('owner')}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.cardTitle, { color: colors.text, fontFamily: 'Inter_700Bold' }]}>
            {business ? 'Request Business Profile Update' : 'Business Registration'}
          </Text>
          {!business && (
            <Text style={{ color: colors.mutedForeground }}>
              Businesses are registered by admins only. Contact an admin to create your business profile.
            </Text>
          )}

          <View style={styles.field}>
            <Text style={[styles.label, { color: colors.text, fontFamily: 'Inter_500Medium' }]}>{t('businessName')}</Text>
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: colors.text, fontFamily: 'Inter_500Medium' }]}>{t('category')}</Text>
                  ...
                  <Text
                    style={[
                      styles.catText,
                      {
                        ...
                        fontFamily: form.category === cat ? 'Inter_600SemiBold' : 'Inter_400Regular',
                      },
                    ]}
                  >
                    {cat}
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: colors.text, fontFamily: 'Inter_500Medium' }]}>{t('bio')}</Text>
            <View style={[styles.textAreaWrap, { borderColor: colors.border, backgroundColor: colors.background }]}>
              <TextInput
                style={[styles.textArea, { color: colors.text, fontFamily: 'Inter_400Regular' }]}
                value={form.bio}
          ...
          <Pressable
            style={({ pressed }) => [styles.saveBtn, { backgroundColor: colors.primary, opacity: pressed || saving || !business ? 0.85 : 1 }]}
            onPress={() => requestBusinessUpdate()}
            disabled={saving || !business}
          >
            {saving ? <ActivityIndicator size="small" color="#fff" /> : (
              <Text style={[styles.saveBtnText, { fontFamily: 'Inter_700Bold' }]}>
                {business ? 'Submit for Approval' : 'Admin Required'}
              </Text>
            )}
          </Pressable>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.cardTitle, { color: colors.text, fontFamily: 'Inter_700Bold' }]}>{t('language')}</Text>
          <View style={styles.langRow}>
            {(['en', 'ur'] as const).map((lang) => (
              <Pressable
                key={lang}
                style={[styles.langBtn, { backgroundColor: language === lang ? colors.primary : colors.muted, borderColor: language === lang ? colors.primary : colors.border }]}
                onPress={() => setLanguage(lang)}
              >
                <Text style={[styles.langText, { color: language === lang ? '#fff' : colors.text, fontFamily: language === lang ? 'Inter_600SemiBold' : 'Inter_400Regular' }]}>
        ...
        <Pressable
          style={({ pressed }) => [styles.signOutBtn, { backgroundColor: colors.destructive + '12', borderColor: colors.destructive + '30', opacity: pressed ? 0.8 : 1 }]}
          onPress={() =>
        ...
        >
          ...
          <Text style={[styles.signOutText, { color: colors.destructive, fontFamily: 'Inter_600SemiBold' }]}>{t('signOut')}</Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  field: { gap: 8 },
  label: { fontSize: 14 },
  inputWrap: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 12, gap: 10 },
  ...
  langBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 10, borderWidth: 1 },
  langText: { fontSize: 15 },
  signOutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingVertical: 15, borderRadius: 14, borderWidth: 1 },
