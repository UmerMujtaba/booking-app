  ...
  return (
    ...
    >
      ...
      <View style={styles.avatarSection}>
        ...
        <View style={[styles.roleBadge, { backgroundColor: colors.primary + '14' }]}>
      ...
      <View style={styles.section}>
        <SettingRow icon="mail" label="Email" value={profile?.phone ?? ''} colors={colors} />
        <SettingRow icon="phone" label={t('phone')} value={profile?.phone ?? 'Not set'} colors={colors} />
      </View>
              ...
              <Text
                style={[
                  styles.langText,
                  {
                    ...
                    fontFamily: language === lang ? 'Inter_600SemiBold' : 'Inter_400Regular',
                  },
                ]}
              >
...
function SettingRow({ icon, label, value, colors }: { icon: string; label: string; value: string; colors: ReturnType<typeof import('@/hooks/useColors').useColors> }) {
  return (
    <View style={[styles.settingRow, { borderBottomColor: colors.border }]}>
      <Feather name={icon as keyof typeof Feather.glyphMap} size={18} color={colors.mutedForeground} />
      <Text style={[styles.settingLabel, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>{label}</Text>
      <Text style={[styles.settingValue, { color: colors.text, fontFamily: 'Inter_500Medium' }]} numberOfLines={1}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  signOutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingVertical: 15, borderRadius: 14, borderWidth: 1, marginTop: 8 },
